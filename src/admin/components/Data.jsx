...
import { FaUsers, FaUser, FaUserPen } from 'react-icons/fa6';
...
const Data = () => {
    ...
    const data = [
        {
            id: 1,
            icon: <FaUsers className="text-8xl mr-4" />,
            number: 500,
            label: "จำนวนผู้ใช้ทั้งหมด"
        },
        {
            id: 2,
            icon: <FaUser className="text-8xl mr-4" />,
            number: 122,
            label: "ผู้สอน"
        },
        {
            id: 3,
            icon: <FaUserPen className="text-8xl mr-4" />,
            number: 321,
            label: "ผู้เรียน"
        }
    ];
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {data.map((item) => (
            <div
                key={item.id}
                className="bg-gradient-to-b from-orange-500 to-purple-500 text-white p-6 rounded-lg flex flex-row items-center justify-between"
            >
                {item.icon}
                <div className="flex flex-col items-end">
                    <span className="text-4xl md:text-5xl lg:text-6xl">{item.number}</span>
                    <span className="text-lg md:text-xl mt-2">{item.label}</span>
                </div>
            </div>
        ))}
    </div>
